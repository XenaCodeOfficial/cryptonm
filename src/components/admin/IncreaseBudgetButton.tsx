...
export default function IncreaseBudgetButton({ clientId, currentBudget }: { clientId: string, currentBudget: number }) {
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                {amount && !isNaN(parseFloat(amount)) && parseFloat(amount) > 0 && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                    Nouveau budget: <span className="font-semibold text-green-600 dark:text-green-400">
                      {formatAmount(currentBudget + parseFloat(amount))}
                    </span>
                  </p>
                )}
              </div>
              ...
              <div className="flex gap-3 justify-end">
                ...
                <button
                  ...
                  disabled={loading}
                >
                  {loading ? 'Enregistrement...' : 'Augmenter'}
                </button>
              </div>
            </form>
          </div>
